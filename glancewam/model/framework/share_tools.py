...
def select_backbone_features(hidden_states, extract_layers=None, combine="concat_tokens"):
    """
    Pick and combine features from chosen backbone layers for the action head.

    Several recent VLA / world-model studies report that the *final* transformer
    layer is not the most informative source for action decoding — intermediate
    layers often carry stronger spatial / dynamics features. This helper lets a
    framework extract one or several layers from a HuggingFace
    ``output_hidden_states=True`` tuple and merge them into a single conditioning
    tensor, instead of hardcoding ``hidden_states[-1]``.

    Args:
        hidden_states: tuple/list of ``[B, N, H]`` tensors from
            ``outputs.hidden_states``. Index ``0`` is the embedding output and
            index ``-1`` the final block, so there are ``num_layers + 1`` entries.
        extract_layers: list of layer indices (negatives allowed). ``None`` or
            ``[-1]`` reproduces the legacy last-layer behaviour. The string
            ``"all"`` selects every entry of ``hidden_states`` in order — use
            this when the backbone has already filtered the tuple to the layers
            it captured (e.g. CosmoPredict2's hook-based extraction), so the
            framework only needs to merge what it received without re-indexing.
        combine: how to merge when more than one layer is requested:
            - ``"concat_tokens"`` : concat along the sequence axis → ``[B, k*N, H]``
              (default; preserves feature dim, so ``cross_attention_dim`` is unchanged)
            - ``"mean"``          : average the layers → ``[B, N, H]``
            - ``"sum"``           : sum the layers → ``[B, N, H]``

    Returns:
        A single ``[B, N', H]`` tensor. The feature dim ``H`` is preserved for
        every ``combine`` mode, so the action head's ``cross_attention_dim``
        alignment stays valid regardless of how many layers are selected.
    """
    n = len(hidden_states)
    if extract_layers is None:
        extract_layers = [-1]
    elif extract_layers == "all":
        extract_layers = list(range(n))
    ...
    for idx in extract_layers:
        ...
        if i < -n or i >= n:
            raise IndexError(
                f"extract_layers index {idx} out of range for a backbone with "
                f"{n} hidden_states (valid: [-{n}, {n - 1}])"
            )
        selected.append(hidden_states[i])

    if len(selected) == 1:
        return selected[0]

    if combine == "concat_tokens":
        return torch.cat(selected, dim=1)
    elif combine == "mean":
        return torch.stack(selected, dim=0).mean(dim=0)
    elif combine == "sum":
        return torch.stack(selected, dim=0).sum(dim=0)
    raise ValueError(f"Unknown combine mode {combine!r}; expected 'concat_tokens', 'mean', or 'sum'.")
...
def apply_config_compat(cfg, *, strict: bool = False):
    """
    Normalise an arbitrary (old or new) glancewam training config into the
    ...
    Performed transformations (each applied only when needed):
      ...
      5.  `framework.action_model.past_action_window_size`
          - Auto-filled to `0` when missing. All released glancewam frameworks
