...
from glancewam.model.framework.share_tools import dict_to_namespace, read_mode_config
...
def _auto_import_framework_modules() -> None:
    ...
    for _, module_name, is_pkg in pkgutil.iter_modules([str(framework_dir)]):
        if module_name in _SKIP:
            continue
        if is_pkg:
            # Scan sub-packages (wam/, etc.)
            sub_dir = framework_dir / module_name
            for _, sub_name, _ in pkgutil.iter_modules([str(sub_dir)]):
                if sub_name.startswith("_"):
                    continue
                importlib.import_module(f"glancewam.model.framework.{module_name}.{sub_name}")
        else:
            importlib.import_module(f"glancewam.model.framework.{module_name}")
...
class baseframework(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        recon = tuple(getattr(FrameworkModel, "_reconstructable_prefixes", ()) or ())
        if recon:
            # Whitelist checkpoints (see TrainerUtils.filter_savable_state_dict) omit frozen,
            # base-reconstructable modules — text_encoder / vae — that the build already
            # populated from base_wm / base_vlm; legacy bloated checkpoints may instead carry
            # them as extras. Tolerate mismatches in BOTH directions under the reconstructable
            # prefixes, but stay strict about every key the framework actually owns.
            result = FrameworkModel.load_state_dict(model_state_dict, strict=False)
            missing_keys = [k for k in result.missing_keys if not k.startswith(recon)]
            unexpected_keys = [k for k in result.unexpected_keys if not k.startswith(recon)]
            if missing_keys:
            ...
            if missing_keys or unexpected_keys:
                raise RuntimeError(
                    f"state_dict mismatch for {type(FrameworkModel).__name__} (excluding reconstructable "
                    f"prefixes {recon}): {len(missing_keys)} missing, {len(unexpected_keys)} unexpected."
                )
        else:
            # Legacy strict path for frameworks that save the full model.
