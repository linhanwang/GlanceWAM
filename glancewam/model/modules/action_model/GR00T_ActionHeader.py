...
# Modification: [rm and add some connect adapter to match with glancewam, e.g., "rm "].
...
from glancewam.model.modules.action_model.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from glancewam.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT
...
class FlowmatchingActionHead(nn.Module):
    def __init__(
        ...
        # ------------------------------------------------------------------
        # Optional Gaussian-bell per-timestep loss weighting (default OFF).
        #   With `t_loss_weighting: true`, each sample's flow-matching MSE is
        #   scaled by a bell over the (discretised) timestep -- peaked at the
        #   mid-noise bucket (t~=0.5) and tapering to ~0 at the clean (t->1) and
        #   pure-noise (t->0) extremes -- so gradient concentrates on the
        #   informative mid-noise regime. Mirrors the `bsmntw_weighing`
        #   schedule the Wan-family video+action co-training repos use
        #   (FastWAM / lingbot-va); see
        #   Weights are
        #   precomputed over the uniform bucket grid and renormalised to mean
        #   1, so the loss magnitude stays comparable to the unweighted path.
        #   Default-off keeps every other GR00T-head caller bit-exact.
        # ------------------------------------------------------------------
        self.t_loss_weighting = bool(config.get("t_loss_weighting", False))
        if self.t_loss_weighting:
            grid = torch.arange(self.num_timestep_buckets, dtype=torch.float32) / self.num_timestep_buckets
            bell = torch.exp(-2.0 * (grid - 0.5) ** 2)
            bell = bell - bell.min()
            bell = bell * (self.num_timestep_buckets / bell.sum())  # mean 1 over the grid
            self.register_buffer("t_loss_weights", bell, persistent=False)
    ...
    def forward(
        ...
        # Upcast to fp32 for the MSE reduction so bf16-autocast callers (e.g.
        # QwenGR00T) keep a numerically stable loss accumulator; a no-op when
        # the caller already runs the head in fp32.
        se = (pred_actions.float() - velocity.float()) ** 2
        if self.t_loss_weighting:
            # Per-sample MSE scaled by the Gaussian-bell weight at that sample's timestep
            # bucket (see __init__). t_discretized is (B,), one bucket per example.
            #
            # Clamp into [0, num_timestep_buckets-1] before the table gather:
            # t_discretized = (t * num_timestep_buckets).long() with t~=noise_s can round
            # to exactly num_timestep_buckets under bf16 (999 is not representable; it
            # rounds up to 1000), which is in-bounds for the sinusoidal timestep encoders
            # that consume it elsewhere but OOB for this lookup table.
            per_sample = se.flatten(start_dim=1).mean(dim=1)  # (B,)
            buckets = t_discretized.clamp(0, self.num_timestep_buckets - 1)
            per_sample = per_sample * self.t_loss_weights[buckets].to(per_sample.dtype)
    ...
    def predict_action(
        ...
        seeds: optional per-ROW sampling seeds (len == batch). When given, the initial
        flow noise for row i is drawn from a CPU generator seeded with seeds[i], making
        the sample reproducible independently of batch composition. seeds=None (the
        default) keeps the original unseeded draw bit-identical.
        """
        # Set initial actions as the sampled noise.
        batch_size = vl_embs.shape[0]
        device = vl_embs.device
        actions = seeded_randn(
...
def seeded_randn(
    ...
    *,
    ...
    """Draw `shape` noise, per-ROW reproducible when `seeds` is given.

    shape[0] is the batch dim. With seeds=None this is exactly `torch.randn(shape)` —
    the historical unseeded path, kept bit-identical. With seeds, row i is drawn from a
    CPU generator seeded with seeds[i], so a row's sample depends only on its own seed
    and NOT on how the server happened to coalesce concurrent requests into a batch.
    (CPU generator then .to(device): CUDA RNG streams are not reproducible across batch
    shapes, which would defeat the point.) Individual None entries fall back to an
    unseeded draw for that row, so a partially-seeded batch is legal.
    """
    if seeds is None:
        return torch.randn(size=shape, dtype=dtype, device=device)
    if len(seeds) != shape[0]:
        raise ValueError(f"seeds length {len(seeds)} != batch size {shape[0]}")
    rows = []
    for s in seeds:
        if s is None:
            rows.append(torch.randn(shape[1:], dtype=torch.float32))
        else:
            g = torch.Generator(device="cpu").manual_seed(int(s) % (2**63 - 1))
            rows.append(torch.randn(shape[1:], generator=g, dtype=torch.float32))
