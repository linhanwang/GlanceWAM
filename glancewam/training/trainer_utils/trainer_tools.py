...
def build_param_lr_groups(model, cfg):
    ...
    if not isinstance(freeze_modules, str):
        if freeze_modules:
            print(
                f"⚠️ trainer.freeze_modules={freeze_modules!r} is not a string and freezes NOTHING; "
                "use a comma-separated list of module-path prefixes (e.g. 'qwen_vl_interface') or ''."
            )
        freeze_modules = ""
    ...
    for module_name, lr in lr_cfg.items():
        ...
        try:
            for attr in module_name.split("."):
                module = getattr(module, attr)
            # filter out frozen parameters (freeze_modules prefixes + any param
            # already requires_grad=False, e.g. a PEFT-LoRA-frozen backbone — so
            # ZeRO-2 never allocates optimizer state for the frozen base).
...
class TrainerUtils:
    @staticmethod
    def filter_savable_state_dict(model, state_dict):
        """Restrict a checkpoint to the framework's declared owned components.

        If ``model`` declares ``_save_include_prefixes`` (a tuple of key prefixes it
        owns and trains, e.g. ``("backbone.transformer.", "action_model.")``), the
        saved state_dict is filtered to exactly those keys. Everything else — notably
        frozen, base-reconstructable modules that get lazily registered for the in-loop
        eval monitor (``backbone.text_encoder.*`` / ``backbone.vae.*`` via
        ``ensure_eval_encoders``) — is dropped, so the on-disk format is invariant
        regardless of whether the monitor ran. Frameworks reconstruct those modules from
        ``base_wm`` / ``base_vlm`` at load (see ``_reconstructable_prefixes`` and
        ``baseframework.from_pretrained``).

        Guard: every trainable parameter MUST be covered by the whitelist; otherwise a
        trained weight would be silently dropped, so we raise instead. Frameworks that do
        not declare ``_save_include_prefixes`` keep the legacy behaviour (save everything).
        """
        include = getattr(model, "_save_include_prefixes", None)
        if not include:
            return state_dict
        include = tuple(include)
        uncovered = [n for n, p in model.named_parameters() if p.requires_grad and not n.startswith(include)]
        if uncovered:
            raise RuntimeError(
                f"_save_include_prefixes={include} does not cover {len(uncovered)} trainable "
                f"parameter(s) — saving would silently drop them. Examples: {uncovered[:5]}. "
                f"Add the missing prefix(es) to the framework's _save_include_prefixes."
            )
        kept = {k: v for k, v in state_dict.items() if k.startswith(include)}
        dropped = len(state_dict) - len(kept)
        if dropped:
            logger.info(f">> [*] checkpoint whitelist kept {len(kept)}/{len(state_dict)} keys (dropped {dropped}).")
        return kept
    ...
    def freeze_backbones(model, freeze_modules=""):
        ...
        if freeze_modules and not isinstance(freeze_modules, str):
            print(
                f"⚠️ trainer.freeze_modules={freeze_modules!r} is not a string and freezes NOTHING; "
                "use a comma-separated list of module-path prefixes (e.g. 'qwen_vl_interface') or ''."
            )
