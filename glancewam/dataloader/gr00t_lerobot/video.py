...
# Per-process LRU cache of torchcodec VideoDecoder objects keyed by video path.
# `VideoDecoder()` eagerly parses the entire frame index, which is ~50–135 ms on
# LeRobot-v3 chunked files (30k+ frames per file). Reusing decoders across the
# many random-access reads a worker performs on the same file turns that into a
# one-time cost. Each dataloader worker has its own cache (forked module state).
# Cap is sized for shuffled access across many chunk files; tunable via env.
_TORCHCODEC_DECODER_CACHE_MAX = int(os.environ.get("GLANCEWAM_TORCHCODEC_CACHE_MAX", "128"))
# `1` keeps decode parallelism at the DataLoader-worker level — empirically faster
# than auto-threading for the small (~16-frame window) reads this loader does, and
# avoids oversubscription with N workers × N ranks on shared hosts. Override via
# env when running with very few workers on a many-core host.
_TORCHCODEC_FFMPEG_THREADS = int(os.environ.get("GLANCEWAM_TORCHCODEC_FFMPEG_THREADS", "1"))
_torchcodec_decoder_cache: "OrderedDict[str, object]" = OrderedDict()


def _get_cached_torchcodec_decoder(video_path: str):
    if not TORCHCODEC_AVAILABLE:
        raise ImportError("torchcodec is not available.")
    cached = _torchcodec_decoder_cache.get(video_path)
    if cached is not None:
        _torchcodec_decoder_cache.move_to_end(video_path)
        return cached
    decoder = torchcodec.decoders.VideoDecoder(
        video_path,
        device="cpu",
        dimension_order="NHWC",
        num_ffmpeg_threads=_TORCHCODEC_FFMPEG_THREADS,
    )
    _torchcodec_decoder_cache[video_path] = decoder
    if len(_torchcodec_decoder_cache) > _TORCHCODEC_DECODER_CACHE_MAX:
...
def get_frames_by_indices(
    ...
    if video_backend == "decord":
        ...
        decoder = _get_cached_torchcodec_decoder(video_path)
