...
class VideoJpeg(VideoTransform):
    """Round-trip frames through JPEG (PIL, RGB) to inject compression artifacts.

    Mirrors cosmos-policy's ``use_jpeg_compression`` (``apply_jpeg_compression_np``,
    PIL ``Image.save(format="JPEG", quality=95)``). Applied in BOTH train and eval —
    JPEG is deterministic, not a random augmentation. Operates on numpy ``[T, H, W, C]``
    uint8, so place it AFTER ``VideoToNumpy`` in the chain. Does not flip or reorder
    channels (input/output are RGB), so it is orientation-agnostic.
    """

    quality: int = Field(default=95, description="JPEG quality factor (1-95)")

    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        return self._make_jpeg(self.quality)

    @staticmethod
    def _make_jpeg(quality: int) -> Callable:
        def _jpeg(frames: np.ndarray) -> np.ndarray:
            assert (
                frames.ndim == 4 and frames.shape[-1] == 3
            ), f"VideoJpeg expects [T, H, W, C] uint8 (place after VideoToNumpy), got shape {frames.shape}"
            out = np.empty_like(frames)
            for i in range(frames.shape[0]):
                buf = io.BytesIO()
                Image.fromarray(frames[i]).save(buf, format="JPEG", quality=quality)
                buf.seek(0)
                out[i] = np.asarray(Image.open(buf))
            return out
    ...
    def check_input(self, data: dict[str, Any]):
        for key in self.apply_to:
            assert key in data, f"Key {key} not found in data. Available keys: {data.keys()}"
            assert data[key].dtype == np.uint8, f"Video {key} must be uint8, got {data[key].dtype}"
            assert data[key].ndim == 4, f"Video {key} must be [T, H, W, C], got ndim {data[key].ndim}"
